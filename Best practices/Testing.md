...
Testing is a vitally important part of creating applications that are meant to withstand the test of time. Tests serve two primary purposes:
...
Note that the meaning and size of “the interface” will depend greatly on the type of test, but that this rule applies regardless. When running unit tests, the interface is mostly comprised of objects and functions. When running e2e tests, the interface includes the URL structure and user interface events.
...
All tests should follow a common pattern. First, do any necessary setup work (for example, instantiating an object, or mocking a dependency to return a particular value). Then, perform the action you are testing (for example, triggering a particular method). Finally, assert on the result, either on the value received from performing the action, or on the side effects that are expected as part of it.

If you find that tests are getting quite large, it can make sense to break common parts of setup/ perform into utility functions scoped to the test file. However, you should still have each test individually call these functions, rather than calling them in a `beforeEach` block, as doing so makes it easy to understand any given test in isolation (this is an example of [preferring explicit over automatic](https://github.com/Shopify/web-foundation/blob/master/Principles/3%20-%20Explicit%20over%20automatic.md)). You should not break assertions out into utility functions; these are the key part of your test, and if you have common assertions across tests (or many assertions per test), it is likely that you are testing too much in each test case (if the assertion itself is complex, consider using a custom assertion, which most frameworks support).

Avoid using mocks (for example, via `sinon.mock()`). Mocks invert the order, putting the assertion first in the test, which makes them harder to read.

### Tests should read well in isolation
...
* Never rely on test ordering for your tests to run successfully.
...
## Testing common frameworks and technologies
